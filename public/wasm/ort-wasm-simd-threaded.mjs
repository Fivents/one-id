var ortWasmThreaded = (() => {
  ...
  return async function (moduleArg = {}) {
    ...
    if (B) {
      ...
      !w.thisProgram && 1 < process.argv.length && (la = process.argv[1].replace(/\\/g, '/'));
      ...
      (A
        ...
        F.startsWith('blob:') ? (F = '') : (F = F.substr(0, F.replace(/[?#].*/, '').lastIndexOf('/') + 1)),
        B ||
          ((ma = (a) => {
          ...
          if ('load' === d) {
            ...
            'cancel' === d
              ...
              : 'setimmediate' !== c.target &&
                ('checkMailbox' === d ? ua && Ca() : d && (I(`worker: received unknown command ${d}`), I(c)));
        } catch (f) {
          throw (Da(), f);
        }
      }
    ...
    var Tb = (a) => {
      ...
      Wb = (a) => {
        ...
        T[a.Ra] = b;
        b.Ra = a.Ra;
        var c = { cmd: 'run', start_routine: a.cb, arg: a.ab, pthread_ptr: a.Ra };
        B && b.unref();
      ...
      U = 0,
      V = (a, b, ...c) => {
        for (var d = 2 * c.length, f = $b(), h = ac(8 * d), k = h >>> 3, q = 0; q < c.length; q++) {
          var y = c[q];
          'bigint' == typeof y
      ...
      new Promise((b) => {
        a.onmessage = (h) => {
          ...
          if (h.targetThread && h.targetThread != K()) {
            ...
            ((h = h.thread), (k = T[h]), delete T[h], Tb(k), hc(h), S.splice(S.indexOf(k), 1), (k.Ra = 0));
          else if ('cancelThread' === k) T[h.thread].postMessage({ cmd: 'cancel' });
          else if ('loaded' === k) ((a.loaded = !0), B && !a.Ra && a.unref(), b(a));
        ...
        a.postMessage({ cmd: 'load', handlers: c, wasmMemory: g, wasmModule: Ea });
      });
    function gc(a) {
    ...
    var rc = 'undefined' != typeof TextDecoder ? new TextDecoder('utf8') : void 0,
      sc = (a, b, c) => {
        b >>>= 0;
        var d = b + c;
        for (c = b; a[c] && !(c >= d); ) ++c;
        if (16 < c - b && a.buffer && rc)
          return rc.decode(a.buffer instanceof SharedArrayBuffer ? a.slice(b, c) : a.subarray(b, c));
        for (d = ''; b < c; ) {
          var f = a[b++];
          if (f & 128) {
            var h = a[b++] & 63;
    ...
    function xa(a) {
      ...
      'function' === typeof Atomics.jb &&
        (Atomics.jb(r(), a >>> 2, a).value.then(Ca), (a += 128), Atomics.store(r(), a >>> 2, 1));
    }
    var Ca = () => {
      var a = K();
      if (a && (xa(a), (a = vc), !Fa))
    ...
    function mb(a, b, c, d, f) {
      ...
      wc.length = d;
      c = (f >>> 0) >>> 3;
      for (f = 0; f < d; f++) wc[f] = N[c + 2 * f] ? N[c + 2 * f + 1] : ea()[(c + 2 * f + 1) >>> 0];
    ...
    function qb(a, b) {
      ...
      r()[((b + 24) >>> 2) >>> 0] = a.getDay();
      var c = ((X(a.getFullYear()) ? yc : zc)[a.getMonth()] + a.getDate() - 1) | 0;
      r()[((b + 28) >>> 2) >>> 0] = c;
    ...
    function rb(a) {
      ...
      0 > c
        ...
        : 0 < c != (k == d) && ((f = Math.max(h, f)), b.setTime(b.getTime() + 6e4 * ((0 < c ? k : f) - d)));
      r()[((a + 24) >>> 2) >>> 0] = b.getDay();
    ...
    function ub(a, b, c, d) {
      ...
      u()[(a >>> 2) >>> 0] = 60 * y;
      r()[(b >>> 2) >>> 0] = Number(f != q);
      a = (v) => v.toLocaleTimeString(void 0, { hour12: !1, timeZoneName: 'short' }).split(' ')[1];
      h = a(h);
      k = a(k);
    ...
    var Cb = () => (B ? require('os').cpus().length : navigator.hardwareConcurrency);
    function Db() {
      La('Cannot use emscripten_pc_get_function without -sUSE_OFFSET_CONVERTER');
      return 0;
    }
    function Eb(a) {
      a >>>= 0;
      ...
      for (var c = 1; 4 >= c; c *= 2) {
        ...
        d = Math.max(a, d);
        a: {
          f = (f.min.call(f, 4294901760, d + ((65536 - (d % 65536)) % 65536)) - g.buffer.byteLength + 65535) / 65536;
          try {
            g.grow(f);
            m();
            var h = 1;
    ...
    var Bc = () => {
        La('Cannot use convertFrameToPC (needed by __builtin_return_address) without -sUSE_OFFSET_CONVERTER');
        return 0;
      },
      Y = {},
      Cc = (a) => {
    ...
    function Nb(a, b, c, d) {
      ...
      for (var f = 0, h = 0; h < c; h++) {
        ...
        for (var y = 0; y < q; y++) {
          var v = n()[(k + y) >>> 0],
            z = Gc[a];
          0 === v || 10 === v ? ((1 === a ? ra : I)(sc(z, 0)), (z.length = 0)) : z.push(v);
        }
        f += q;
      }
      u()[(d >>> 2) >>> 0] = f;
    ...
    function Pb(a, b, c, d) {
      ...
      function y(e) {
        ...
        t = q(t);
        return 0 >= k(p, e) ? (0 >= k(t, e) ? e.getFullYear() + 1 : e.getFullYear()) : e.getFullYear() - 1;
      }
      a >>>= 0;
      b >>>= 0;
      c >>>= 0;
      ...
      var Yb = 'Sunday Monday Tuesday Wednesday Thursday Friday Saturday'.split(' '),
        Zb = 'January February March April May June July August September October November December'.split(' ');
      v = {
        ...
        '%j': (e) => {
          for (var p = 0, t = 0; t <= e.Wa - 1; p += (X(e.Ta + 1900) ? Hc : Ic)[t++]);
          return h(e.Ya + p, 3);
        },
        '%m': (e) => h(e.Wa + 1, 2),
        '%M': (e) => h(e.fb, 2),
        ...
        '%V': (e) => {
          ...
          if (p) 53 == p && ((t = (e.Qa + 371 - e.Sa) % 7), 4 == t || (3 == t && X(e.Ta)) || (p = 1));
          else {
            p = 52;
      ...
      for (z in v) c.includes(z) && (c = c.replace(new RegExp(z, 'g'), v[z](d)));
      c = c.replace(/\0\0/g, '%');
      z = Jc(c);
    ...
    C || fc();
    var xc = [Ob, dc, qc, Wa, Xa, Ya, Za, $a, ab, bb, cb, db, eb, fb, gb, hb, sb, tb, Hb, Ib, Kb, Lb, Mb, Nb],
      Sa,
      Z = (function () {
        function a(c, d) {
          Z = c.exports;
    ...
    w._OrtAppendExecutionProvider = (a, b) => (w._OrtAppendExecutionProvider = Z.da)(a, b);
    w._OrtAddFreeDimensionOverride = (a, b, c) => (w._OrtAddFreeDimensionOverride = Z.ea)(a, b, c);
    w._OrtAddSessionConfigEntry = (a, b, c) => (w._OrtAddSessionConfigEntry = Z.fa)(a, b, c);
    w._OrtReleaseSessionOptions = (a) => (w._OrtReleaseSessionOptions = Z.ga)(a);
    w._OrtCreateSession = (a, b, c) => (w._OrtCreateSession = Z.ha)(a, b, c);
    w._OrtReleaseSession = (a) => (w._OrtReleaseSession = Z.ia)(a);
    w._OrtGetInputOutputCount = (a, b, c) => (w._OrtGetInputOutputCount = Z.ja)(a, b, c);
    ...
    w._OrtCreateTensor = (a, b, c, d, f, h) => (w._OrtCreateTensor = Z.na)(a, b, c, d, f, h);
    w._OrtGetTensorData = (a, b, c, d, f) => (w._OrtGetTensorData = Z.oa)(a, b, c, d, f);
    w._OrtReleaseTensor = (a) => (w._OrtReleaseTensor = Z.pa)(a);
    w._OrtCreateRunOptions = (a, b, c, d) => (w._OrtCreateRunOptions = Z.qa)(a, b, c, d);
    w._OrtAddRunConfigEntry = (a, b, c) => (w._OrtAddRunConfigEntry = Z.ra)(a, b, c);
    ...
    w._OrtRunWithBinding = (a, b, c, d, f) => (w._OrtRunWithBinding = Z.ya)(a, b, c, d, f);
    w._OrtRun = (a, b, c, d, f, h, k, q) => (w._OrtRun = Z.za)(a, b, c, d, f, h, k, q);
    ...
    function Oc() {
      0 < O ||
        (C
          ...
          : (jc(Ga), 0 < O || Mc || ((Mc = !0), (w.calledRun = !0), Fa || (C || jc(Ha), ha(w), C || jc(Ia)))));
    }
    Oc();
    moduleRtn = ia;
