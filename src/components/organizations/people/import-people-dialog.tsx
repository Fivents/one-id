...
import { excelPeople, type ValidationError } from '@/core/utils/excel-people';
...
export function ImportPeopleDialog({ open, onOpenChange, organizationId }: ImportPeopleDialogProps) {
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [file, setFile] = useState<File | null>(null);
  const [overwrite, setOverwrite] = useState(false);
  const [preview, setPreview] = useState<{ nome: string; email: string; cpf: string }[]>([]);
  ...
  const handleFileChange = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const result = await excelPeople.parseFromExcel(selectedFile);
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      const result = await excelPeople.parseFromExcel(file);
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Erro ao importar pessoas.');
      }
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>Faça upload de uma planilha para importar pessoas.</DialogDescription>
          </DialogHeader>
