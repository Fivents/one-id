...
import { updateClientUserRequestSchema } from '@/core/communication/requests/admin';
...
export const PATCH = withAuth(
  withSuperAdmin(async (req: NextRequest, context: RouteContext) => {
    try {
      ...
      const data = parseWithZod(updateClientUserRequestSchema, body);

      const controller = makeUpdateClientUserController();
...
export const DELETE = withAuth(
  withSuperAdmin(async (_req: NextRequest, context: RouteContext) => {
    try {
      ...
      const controller = makeDeleteClientUserController();
