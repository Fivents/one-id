...
import { makeCreateClientUserController, makeListAdminUsersController } from '@/core/application/controller-factories';
...
export const GET = withAuth(
  withSuperAdmin(async () => {
    const controller = makeListAdminUsersController();
...
export const POST = withAuth(
  withSuperAdmin(async (req: NextRequest) => {
    try {
      ...
      const data = parseWithZod(createClientUserRequestSchema, body);
